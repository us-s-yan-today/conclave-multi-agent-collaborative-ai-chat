...
import { getAgents } from '../src/lib/agents';
const MAX_MESSAGES = 40; // Limit to last 20 user/assistant pairs
/**
...
export class ChatAgent extends Agent<Env, ChatState> {
  ...
  async onStart(): Promise<void> {
    ...
    const agent = agents.find(a => a.model === this.state.model) || agents[0];
  ...
  private async handleChatMessage(body: { message: string; model?: string; stream?: boolean }): Promise<Response> {
    ...
    if (model && model !== this.state.model) {
      ...
      const agents = getAgents();
      const agent = agents.find(a => a.model === model) || agents[0];
      
      this.chatHandler?.updateModel(model, agent.apiConfig);
    }
