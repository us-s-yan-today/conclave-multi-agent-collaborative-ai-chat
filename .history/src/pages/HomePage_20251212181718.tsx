  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const agent of primaryAgents) {
        ...
        // Store the primary agent's response
        const finalMessage = messages.find(m => m.id === streamId);
        if (finalMessage?.content) {
          primaryResponses.push(`${agent.name}: ${finalMessage.content}`);
        }
      ...
      <div className="h-screen w-screen bg-card text-foreground overflow-hidden">
        ...
        <div className="w-full h-full max-w-none px-2 sm:px-4 min-h-0">
          <div className="py-2 h-full min-h-0">
            <div className="grid grid-cols-1 md:grid-cols-[280px,1fr,320px] lg:grid-cols-[320px,1fr,380px] xl:grid-cols-[360px,1fr,420px] h-full min-h-0 gap-2 sm:gap-4">
              {!isMobile && (
                <LeftPanel
                  agents={agents}
                  setAgents={setAgents}
              ...
              />
              {!isMobile && <RightPanel agents={agents} messages={messages} isProcessing={isProcessing} onRetrySummary={handleRetrySummary} />}
            </div>
          </div>
        </div>
        <AgentConfigDrawer
