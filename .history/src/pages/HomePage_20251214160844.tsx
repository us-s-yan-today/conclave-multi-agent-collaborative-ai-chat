...
export function HomePage() {
  ...
  const handleSwitchSession = useCallback(async (sessionId: string) => {
    ...
    const allMessages = res.success && res.data
      ? res.data.messages
      : [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // No need to manually format history - the LLM provider will handle it
    ...
    try {
      ...
      const observerPromises = observerAgents.map(async (agent) => {
        ...
        // Observer agents respond to the primary agent's message
        const primaryContext = primaryResponses.length > 0
          ? primaryResponses.join('\n\n')
          : userInput;
        
        const fullPrompt = `The user asked: "${userInput}"\n\nThe primary agent responded:\n${primaryContext}\n\nAs an observer, provide your perspective or feedback on this response. Start your response with "SEVERITY: NONE/MINOR/IMPORTANT/CRITICAL" based on how urgent your feedback is.`;
        
        try {
          let fullResponse = '';
          const response = await chatService.sendObserverMessage(fullPrompt, agent, (chunk) => {
            fullResponse += chunk;
          });
    ...
    (async () => {
      ...
      try {
        ...
        const observerPromises = observerAgents.map(async (observerAgent) => {
          ...
          const observerPrompt = `The user asked: "${userInput}"\n\nThe primary agent responded:\n${combinedContext}\n\nAs an observer, provide your perspective or feedback on this response. Start your response with "SEVERITY: NONE/MINOR/IMPORTANT/CRITICAL" based on how urgent your feedback is.`;
          
          let fullResponse = '';
          const res = await chatService.sendObserverMessage(observerPrompt, observerAgent, (chunk) => {
            fullResponse += chunk;
          });
  ...
  const handleExportSession = useCallback(async (sessionId: string) => {
    try {
      ...
      // Export to YAML
      const yamlContent = exportSessionToYAML(session.id, session.title, sessionMessages, agents, getApiConfigs());
