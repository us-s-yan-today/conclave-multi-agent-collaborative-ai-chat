...
export function HomePage() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Cmd+1-9: Open agent - either observer messages or config
      const num = parseInt(e.key);
      if (num >= 1 && num <= 9) {
        e.preventDefault();
        const agent = agents[num - 1];
        if (agent) {
          // If it's an Observer with pending messages, open the observer drawer
          if (agent.role === 'Observer' && agent.status === 'Hand Raised' && agent.pendingMessages && agent.pendingMessages.length > 0) {
            ...
            // Otherwise, open the config
            handleEditAgent(agent);
            toast.info(`Opening ${agent.name} config`);
          }
        }
        return;
      }
  ...
  }, [agents, handleEditAgent, scrollToBottom]);
