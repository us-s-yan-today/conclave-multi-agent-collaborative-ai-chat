...
import { Agent, getAgents, saveAgents, applySessionStateToAgents, extractSessionStateFromAgents, resetAgentSessionState } from '@/lib/agents';
import { getSessionAgentStates, saveSessionAgentStates, clearSessionAgentStates } from '@/lib/local-store';
import { initializeApiConfigs } from '@/lib/api-configs';
import { exportSessionToYAML, generateSessionExportFilename, downloadYAMLFile } from '@/lib/config-import-export';
...
export function HomePage() {
    ...
    try {
      ...
      const observerPromises = observerAgents.map(async (agent) => {
        ...
        try {
          ...
          // New messages replace old ones (only keep the latest)
          setAgents(prev => produce(prev, draft => {
            const agentToUpdate = draft.find(a => a.id === agent.id);
            if (agentToUpdate) {
              // Replace all pending messages with the new one
    ...
    (async () => {
      ...
      try {
        ...
        for (const primaryAgent of primaryAgents) {
          ...
          const fullPrompt = `[You are ${primaryAgent.name}. Personality: ${primaryAgent.personality}. Previous context:\n${prunedHistory || 'New conversation.'}]\n\nUser: ${userInput}`;
          const streamId = crypto.randomUUID();
          const assistantMessage: ExtendedMessage = {
            id: streamId,
            role: 'assistant',
        ...
        const observerPromises = observerAgents.map(async (observerAgent) => {
          ...
          setAgents(prev => produce(prev, draft => {
            ...
            if (agentToUpdate) {
              ...
              if (output && output !== 'PASS' && output !== '✓' && output.length > 5) {
                const newMessage = { id: crypto.randomUUID(), content: output, timestamp: Date.now() };
  ...
  const handleExportSession = useCallback(async (sessionId: string) => {
    try {
      ...
      // Export to YAML
      const yamlContent = exportSessionToYAML(session, sessionMessages, agentStates, model);
