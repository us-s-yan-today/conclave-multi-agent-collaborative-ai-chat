...
export function exportConfigToYAML(agents: Agent[], apiConfigs: ApiConfig[]): string {
  const configData: ConfigExport = {
    ...
    agents: agents.map(agent => {
      // Remove runtime state from export
    ...
    apiConfigs: apiConfigs.map(config => {
      ...
      return {
        ...configWithoutKey,
        apiKey: '', // Empty API key in export
      };
    })
  };
...
export function exportSessionToYAML(
  ...
  return YAML.stringify(sessionData);
}

/**
 * Export agent configuration to YAML (just agents, no API configs)
 */
