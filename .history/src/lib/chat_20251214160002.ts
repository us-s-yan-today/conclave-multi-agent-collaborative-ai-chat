...
class ChatService {
  ...
  async sendMessage(
    ...
    options?: { visibleInChat?: boolean; includeInHistory?: boolean }
    ...
    const includeInHistory = options?.includeInHistory ?? true;
    ...
    // Only add messages to session state if they should be included in history
    let workingState: ChatState;
    if (includeInHistory) {
      workingState = await saveSessionState(this.sessionId, {
        ...currentState,
        model: agent.model,
      ...
      // For non-history messages (like Observer internal messages), use in-memory state only
      workingState = {
        ...currentState,
        model: agent.model,
        messages: [...currentState.messages, userMessage],
    ...
    try {
      const historyForProvider = workingState.messages.filter(m => m.visibleInChat !== false);
      const content = await this.callModel(message, agent, historyForProvider, onChunk);
      ...
      // Only save final state if messages should be included in history
      let finalState: ChatState;
      if (includeInHistory) {
        finalState = await saveSessionState(this.sessionId, {
          ...workingState,
          messages: [...workingState.messages, assistantMessage],
          isProcessing: false
        });
      } else {
        // For non-history messages, return in-memory state without persisting
        finalState = {
          ...workingState,
          messages: [...workingState.messages, assistantMessage],
          isProcessing: false
      ...
      // Only save error messages if they should be included in history
      let finalState: ChatState;
      if (includeInHistory) {
        finalState = await saveSessionState(this.sessionId, {
          ...workingState,
  ...
  async summarizeConversation(messages: ExtendedMessage[], updateAgentStatus?: (agentId: string, status: 'Thinking' | 'Ready') => void): Promise<string> {
    const visibleMessages = messages.filter(m => m.visibleInChat !== false);
    ...
    const conversationText = visibleMessages.slice(-15).map(m => `${m.agentName || m.role}: ${m.content}`).join('\n');
    ...
    try {
      ...
      if (summarizerAgent) {
        ...
        content = await this.callModel(summaryRequest, summarizerAgent, visibleMessages.slice(-10));
      } else {
        ...
        const response = await chatHandler.processMessage(summaryRequest, visibleMessages.slice(-10));
