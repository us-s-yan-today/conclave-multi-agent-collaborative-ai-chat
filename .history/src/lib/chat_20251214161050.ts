...
class ChatService {
  ...
  async sendObserverMessage(
    ...
    try {
      // Observer agents receive only the formatted prompt, no conversation history
      // The prompt already contains all necessary context about the user's question and primary response
      const content = await this.callModel(message, agent, [], onChunk);
