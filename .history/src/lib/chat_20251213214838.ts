...
class ChatService {
  ...
  async summarizeConversation(messages: ExtendedMessage[], updateAgentStatus?: (agentId: string, status: 'Thinking' | 'Ready') => void): Promise<string> {
    ...
    // Use Summarizer's own prompt if available, otherwise use default summarization prompt
    const defaultPrompt = `Summarize the following conversation concisely. Extract key points and any action items (prefix action items with "ACTION:").\n\nConversation:\n${conversationText}`;
    
    try {
      let content: string;
      
      if (summarizerAgent) {
        // Use Summarizer agent with its configured prompt
        content = await this.callModel(conversationText, summarizerAgent, messages.slice(-10));
      } else {
        // Use fallback agent but override with default summarization prompt
        ...
        const response = await chatHandler.processMessage(defaultPrompt, messages.slice(-10));
