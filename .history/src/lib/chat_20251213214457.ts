...
class ChatService {
  ...
  async summarizeConversation(messages: ExtendedMessage[], updateAgentStatus?: (agentId: string, status: 'Thinking' | 'Ready') => void): Promise<string> {
    // Find the Summarizer agent
    const agents = getAgents();
    const summarizerAgent = agents.find(a => a.role === 'Summarizer' && isAgentConfigValid(a));
    
    if (!summarizerAgent) {
      return 'No Summarizer agent configured. Please assign an agent as Summarizer.';
    }
    ...
    try {
      const content = await this.callModel(prompt, summarizerAgent, messages.slice(-10));
