...
const openDB = (): Promise<IDBDatabase> => {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      // Add new object store for agent states (only if upgrading from version 1 or creating fresh)
      if (oldVersion < 2 && !db.objectStoreNames.contains(STORE_AGENT_STATES)) {
...
export const saveSessionState = async (sessionId: string, state: ChatState): Promise<ChatState> => {
  const prunedMessages = state.messages.slice(-MAX_MESSAGES);
