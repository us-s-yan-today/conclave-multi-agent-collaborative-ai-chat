...
import { getAgents, isAgentConfigValid, validateApiConfig } from './agents';
import {
...
class ChatService {
  ...
  private async callModel(message: string, agent: Agent, history: Message[], onChunk?: (chunk: string) => void): Promise<string> {
    if (!validateApiConfig(agent.apiConfig)) {
      throw new Error('Invalid API configuration');
    }
    if (agent.apiConfig.type === 'gemini') {
      return this.callGemini(message, agent, history, onChunk);
    }
    if (agent.apiConfig.type === 'openai') {
      return this.callOpenAI(message, agent, history, onChunk);
    }
    throw new Error(`Unsupported provider: ${agent.apiConfig.type}`);
  }
