...
// Create default API configuration if none exists
const ensureDefaultApiConfig = (): void => {
  const configs = getApiConfigs();
  
  // Check if any config exists at all, not just a specific ID
  // This prevents duplicate creation on every load
  if (configs.length === 0) {
    const defaultConfig: ApiConfig = {
      id: 'default-api-config',
      name: 'Default Gemini API',
      type: 'gemini',
    ...
    // Directly save instead of using createApiConfig to preserve the ID
...
// Initialize default API config on first load
