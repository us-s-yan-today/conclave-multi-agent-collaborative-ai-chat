...
const ensureDefaultApiConfig = (): void => {
  const configs = getApiConfigs();
  const hasDefault = configs.some(config => config.id === 'default-api-config');
...
export const createApiConfig = (config: Omit<ApiConfig, 'id'>): ApiConfig => {
