...
/**
 * Generate session export filename with timestamp and sanitized title
 */
export const generateSessionExportFilename = (sessionTitle: string): string => {
  ...
  return `session-${sanitizedTitle}-${getTimestampString()}.yaml`;
};

/**
 * Export a complete session to YAML format
 */
export const exportSessionToYAML = (
  session: SessionInfo,
  messages: Message[],
  agentStates: Record<string, AgentSessionState>,
  model: string
): string => {
...
/**
 * Parse YAML session file
 */
export const importSessionFromYAML = (yamlContent: string): SessionExport => {
  try {
    const parsed = yaml.load(yamlContent) as SessionExport;
    ...
    if (!parsed.version || !parsed.session || !parsed.messages) {
      throw new Error('Invalid session file format - missing required fields');
    }
    
    if (!Array.isArray(parsed.messages)) {
      throw new Error('Invalid session file format - messages must be an array');
    }

    if (!parsed.session.id || !parsed.session.title) {
      throw new Error('Invalid session file format - session must have id and title');
    }
    
    return parsed;
  } catch (error) {
    if (error instanceof Error) {
      throw new Error(`Failed to parse YAML session: ${error.message}`);
    }
    throw new Error('Failed to parse YAML session');
  }
};
