          ...
          <div className="w-full max-w-full overflow-hidden">
            ...
            <div className="space-y-1 w-full max-w-full overflow-hidden">
              {sessions.map(session => (
                <div key={session.id} className={cn("group flex items-center p-1.5 rounded-lg cursor-pointer transition-colors hover:bg-accent dark:hover:bg-muted/50 w-full max-w-full overflow-hidden", currentSessionId === session.id && "bg-accent dark:bg-muted/50 border-l-2 border-primary")}>
                  <button onClick={() => onSwitchSession(session.id)} className="flex-1 text-left text-[10px] truncate pr-1 font-medium min-w-0 overflow-hidden">
                    {session.title}
                  </button>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="ghost" size="icon" className="h-5 w-5 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => onDeleteSession(session.id)}>
