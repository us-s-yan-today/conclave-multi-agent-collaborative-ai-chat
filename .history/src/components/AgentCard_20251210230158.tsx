...
export function AgentCard({ agent, onEdit, onDelete, onPromote }: AgentCardProps) {
  ...
  return (
    <div className={cn(
      "flex items-center gap-3 p-2 rounded-lg transition-all duration-200 hover:bg-accent",
      agent.role === 'Primary' && 'bg-accent'
    )}>
      <div className={cn("w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 relative", agent.color)}>
        <IconComponent className="w-5 h-5 text-white" />
        {!hasValidConfig && (
          <AlertTriangle
            className="w-3 h-3 text-yellow-500 absolute -top-1 -right-1 bg-white rounded-full"
