  ...
  const handleImportAgentConfig = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (mappedCount === imported.agents.length) {
        toast.success(`Configuration "${imported.name}" imported with ${imported.agents.length} agents, all mapped to existing API configs!`);
      } else if (mappedCount > 0) {
        toast.success(`Configuration "${imported.name}" imported with ${imported.agents.length} agents (${mappedCount} mapped to existing configs)`);
      } else {
        toast.warning(`Configuration "${imported.name}" imported with ${imported.agents.length} agents. Please configure API settings for the agents.`);
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
