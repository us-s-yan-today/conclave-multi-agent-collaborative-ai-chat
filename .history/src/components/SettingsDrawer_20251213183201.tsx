...
export function SettingsDrawer({ isOpen, onClose, onImportSession }: SettingsDrawerProps) {
  ...
  const handleImportConfig = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Import API configs - check for duplicates based on name and type
  ...
  const handleImportAgentConfig = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const imported = importAgentConfiguration(configData);
      
      setAgentConfigurations(getAgentConfigurations());
      toast.success(`Configuration "${imported.name}" imported with ${imported.agents.length} agents!`);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
