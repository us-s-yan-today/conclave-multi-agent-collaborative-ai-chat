...
export function RightPanel({ agents, messages, isProcessing, onRetrySummary }: RightPanelProps) {
  ...
  const [sheetWidth, setSheetWidth] = useState(50); // Width as percentage of viewport
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      if (isDraggingRef.current) {
        ...
        const newWidthPercent = Math.min(Math.max((distanceFromRight / viewportWidth) * 100, 20), 80);
