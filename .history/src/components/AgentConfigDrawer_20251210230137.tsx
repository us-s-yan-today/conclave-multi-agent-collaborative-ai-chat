import { Agent, agentIcons, getAgentIcon, loadAgentTemplate, TEMPLATE_PRESETS, validateApiConfig } from '@/lib/agents';
...
export function AgentConfigDrawer({ agent, isOpen, onClose, onSave }: AgentConfigDrawerProps) {
  ...
  const handleSave = () => {
    if (currentAgent) {
      ...
      if (!validateApiConfig(currentAgent.apiConfig)) {
        toast.error("Valid Google API configuration is required.");
        return;
      }
      onSave(currentAgent);
      onClose();
  ...
  const isApiConfigValid = currentAgent ? validateApiConfig(currentAgent.apiConfig) : false;
                ...
                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <Input
                      ...
                      placeholder="AIz..."
                      className={!isApiConfigValid && currentAgent.apiConfig.apiKey ? "border-yellow-400" : ""}
                    />
                    {!isApiConfigValid && currentAgent.apiConfig.apiKey && (
                      <p className="text-sm text-yellow-600 mt-1">
                        Google API keys should start with "AI" and be at least 20 characters
                      </p>
                    )}
