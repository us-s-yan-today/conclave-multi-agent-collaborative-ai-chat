...
import { Menu, Users, Send, User, AlertTriangle, X, MessageSquare, FileDown, Lock, RotateCcw, Plus, PanelLeftClose, PanelLeftOpen, PanelRightClose, PanelRightOpen } from 'lucide-react';
...
interface CenterPanelProps {
  ...
  onPromoteAgent: (agentId: string, newRole: 'Primary' | 'Summarizer' | 'Observer') => void;
  ...
  return (
    <main className="flex flex-col h-full min-h-0 bg-background border rounded-lg overflow-hidden">
      <header className="p-4 border-b flex items-center justify-between flex-shrink-0">
          ...
          {isMobile && (
            <Sheet>
              ...
              <SheetContent side="left" className="p-0 w-full max-w-xs">
                <LeftPanel agents={agents} setAgents={setAgents} sessions={sessions} currentSessionId={currentSessionId} onNewSession={onNewSession} onSwitchSession={onSwitchSession} onDeleteSession={onDeleteSession} onEditAgent={onEditAgent} onPromoteAgent={onPromoteAgent} onExportSession={onExportSession} onImportSession={onImportSession} />
              </SheetContent>
            </Sheet>
          )}
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" className="transition-transform hover:scale-105" onClick={onNewSession}>
