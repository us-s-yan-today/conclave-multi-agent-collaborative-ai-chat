...
export function RightPanel({ agents, messages, isProcessing, onRetrySummary }: RightPanelProps) {
  ...
  const [sheetWidth, setSheetWidth] = useState(70); // Width as percentage of viewport - increased from 50
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      if (isDraggingRef.current) {
        ...
        const newWidthPercent = Math.min(Math.max((distanceFromRight / viewportWidth) * 100, 10), 90);
        setSheetWidth(newWidthPercent);
      }
    };
