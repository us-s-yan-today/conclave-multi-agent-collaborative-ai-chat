...
import { Agent, AgentSeverity } from '@/lib/agents';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Bot, CheckCircle, CircleDashed, MessageSquareQuote, ListTodo, BrainCircuit, BarChart2, AlertTriangle, Hand, Star, FileText, Slash, HandMetal } from 'lucide-react';
...
interface RightPanelProps {
  ...
  onUpdateAgentStatus?: (agentId: string, status: 'Thinking' | 'Ready') => void;
}
const StatusIndicator = ({ status, severity }: { status: Agent['status']; severity?: AgentSeverity }) => {
  // Show status-based icon first
  if (status === 'Thinking') return <BrainCircuit className="w-4 h-4 text-yellow-500 animate-pulse" />;
  if (status === 'Paused') return <AlertTriangle className="w-4 h-4 text-red-500" />;
  
  // For Hand Raised status, show severity-based icon
  if (status === 'Hand Raised' && severity) {
    switch (severity) {
      case 'CRITICAL': return <HandMetal className="w-4 h-4 text-red-600 animate-bounce" />; // 2 hands raised
      case 'IMPORTANT': return <Hand className="w-4 h-4 text-amber-600" />; // 1 hand raised
      case 'MINOR': return <Slash className="w-4 h-4 text-blue-500" />; // Shhhh sign
      case 'NONE': return <CheckCircle className="w-4 h-4 text-green-500" />; // Should not show, but fallback
    }
  }
  
  // Default states
  if (status === 'Ready') return <CheckCircle className="w-4 h-4 text-green-500" />;
  if (status === 'Has Feedback') return <MessageSquareQuote className="w-4 h-4 text-blue-500" />;
  
  return <CircleDashed className="w-4 h-4 text-muted-foreground" />;
};
export function RightPanel({ agents, messages, isProcessing, onRetrySummary, onAddObserverMessage, onDismissObserverMessage, onInsertObserverMessageToInput, observerShortcuts, onUpdateAgentStatus }: RightPanelProps) {
  ...
  useEffect(() => {
    ...
    chatService.summarizeConversation(messages.slice(-5), onUpdateAgentStatus)
      .then(setSummary)
  ...
  return (
    <aside className="hidden md:flex flex-col gap-6 h-full min-h-0 p-4 bg-muted/30 rounded-lg overflow-hidden">
                    ...
                    const agentItem = (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant={
                            agent.role === 'Primary' ? 'default' : 
                            agent.role === 'Summarizer' ? 'secondary' : 
                            'outline'
                          }>
                            {agent.role === 'Primary' && <Star className="w-3 h-3 mr-1" />}
                            {agent.role === 'Summarizer' && <FileText className="w-3 h-3 mr-1" />}
                            {agent.role}
                          </Badge>
                          {shortcutNumber && (
                            <Badge variant="outline" className="bg-primary/10 text-primary text-xs px-1.5 py-0 font-mono">
                              ⌘{shortcutNumber}
                            </Badge>
                          )}
                        </div>
                        <div className="flex items-center gap-2 text-muted-foreground">
                          ...
                          <StatusIndicator status={agent.status} severity={agent.severity} />
                        </div>
                      </li>
                    );
                        ...
                        <Tooltip key={agent.id}>
                          ...
                          <TooltipContent side="left" className="font-mono text-xs">
                            <div className="flex flex-col gap-1">
                              <div>⌘{shortcutNumber} - Open message</div>
                              <div>⌘↵ - Add to chat</div>
                              <div>⇧⌘↵ - Insert to input</div>
                              <div>Esc - Close</div>
                            </div>
                          </TooltipContent>
                        </Tooltip>
      ...
      <ObserverMessagesDrawer
        ...
        onInsertToInput={onInsertObserverMessageToInput}
      />
    </aside>
  );
}
