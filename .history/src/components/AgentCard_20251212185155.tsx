...
export function AgentCard({ agent, onEdit, onDelete, onPromote }: AgentCardProps) {
  ...
  return (
    <div className={cn(
      "flex items-center gap-2 sm:gap-3 p-1.5 sm:p-2 rounded-lg transition-all duration-200 hover:bg-accent w-full max-w-full overflow-hidden",
      agent.role === 'Primary' && 'bg-accent'
    )}>
      <div className={cn("w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center flex-shrink-0", agent.color)}>
      ...
      <div className="flex-1 min-w-0 overflow-hidden">
        ...
        <Badge variant={agent.role === 'Primary' ? 'default' : 'secondary'} className="h-4 sm:h-5 text-[10px] sm:text-xs">
          {agent.role === 'Primary' && <Star className="w-2.5 h-2.5 sm:w-3 sm:h-3 mr-0.5 sm:mr-1" />}
          {agent.role}
        </Badge>
      </div>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="icon" className="h-6 w-6 sm:h-8 sm:w-8 flex-shrink-0 focus:ring-2 focus:ring-ring">
