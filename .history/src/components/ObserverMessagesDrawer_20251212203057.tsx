...
interface ObserverMessagesDrawerProps {
  ...
  onInsertToInput: (agentId: string, messageId: string) => void;
}

export function ObserverMessagesDrawer({ isOpen, onClose, agent, onAddToChat, onDismiss, onInsertToInput }: ObserverMessagesDrawerProps) {
  ...
  const handleInsertToInput = () => {
    if (agent && agent.pendingMessages && agent.pendingMessages.length > 0) {
      onInsertToInput(agent.id, agent.pendingMessages[0].id);
      onClose();
    }
  };
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      // Shift+Cmd+Enter or Shift+Ctrl+Enter: Insert to input
  ...
  }, [isOpen, agent, onAddToChat, onDismiss, onInsertToInput, onClose]);
