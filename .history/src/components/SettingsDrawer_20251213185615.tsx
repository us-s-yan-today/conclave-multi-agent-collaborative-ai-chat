...
import { Trash2, Plus, CheckCircle, AlertTriangle, Edit, X, Download, Upload, AlertCircle, Copy } from 'lucide-react';
...
import type { AgentConfiguration } from '@/lib/agent-configs';
...
import {
  ...
  exportAgentConfigToYAML,
  importAgentConfigFromYAML,
  generateAgentConfigExportFilename,
...
export function SettingsDrawer({ isOpen, onClose, onImportSession, onAgentConfigChange }: SettingsDrawerProps) {
  ...
  const agentConfigFileInputRef = useRef<HTMLInputElement>(null);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setAgentConfigurations(getAgentConfigurations());
  ...
  const handleImportConfig = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Create a mapping from imported config characteristics to existing configs
      const apiConfigMap = new Map<string, string>();
      let newConfigsCount = 0;
      let reusedConfigsCount = 0;

      // Import API configs - reuse existing validated configs when possible
      if (importData.apiConfigs && Array.isArray(importData.apiConfigs)) {
        importData.apiConfigs.forEach(importedConfig => {
          // Try to find existing config with same type and baseURL (indicates same API)
          const matchingExisting = existingConfigs.find(
            existing => 
              existing.type === importedConfig.type && 
              existing.baseURL === importedConfig.baseURL &&
              existing.isValidated
          );
          
          if (matchingExisting) {
            // Reuse existing validated config
            apiConfigMap.set(importedConfig.id, matchingExisting.id);
            reusedConfigsCount++;
          } else {
            // Create new config only if no match found
            const newConfig = {
              ...importedConfig,
              id: crypto.randomUUID(),
              // Keep validation status if API key is provided, otherwise reset
              isValidated: importedConfig.apiKey ? importedConfig.isValidated : false,
              lastValidated: importedConfig.apiKey ? importedConfig.lastValidated : undefined,
            };
            createApiConfig(newConfig);
            apiConfigMap.set(importedConfig.id, newConfig.id);
            newConfigsCount++;
          }
        });
        setApiConfigs(getApiConfigs());
      ...
      // Import agents with mapped API config IDs
      if (importData.agents && Array.isArray(importData.agents)) {
        const newAgents = importData.agents.map(agent => ({
          ...agent,
          id: crypto.randomUUID(),
          status: 'Ready' as const,
          // Map to existing or newly created API config
          apiConfigId: apiConfigMap.get(agent.apiConfigId) || agent.apiConfigId,
        }));
        saveAgents(newAgents);
      }

      const totalConfigs = importData.apiConfigs?.length || 0;
      if (reusedConfigsCount > 0) {
        toast.success(
          `Configuration imported! ${importData.agents.length} agents added. ` +
          `Reused ${reusedConfigsCount} existing API config(s), created ${newConfigsCount} new one(s).`
        );
      } else {
        toast.success(
          `Configuration imported! Added ${importData.agents.length} agents and ${newConfigsCount} API config(s).`
        );
      }
    } catch (error) {
  ...
  const handleImportSession = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const content = await readFileAsText(file);
      const sessionData = importSessionFromYAML(content);

      if (!onImportSession) {
        toast.error('Session import not available');
        return;
      }

      await onImportSession(sessionData);
      toast.success(`Session "${sessionData.session.title}" imported successfully!`);
      onClose(); // Close settings drawer after successful import
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      toast.error(`Failed to import session: ${errorMessage}`);
      console.error(error);
    } finally {
      // Reset the input so the same file can be imported again
  ...
  const handleExportAgentConfig = (configId: string) => {
    const config = agentConfigurations.find(c => c.id === configId);
    if (!config) {
    ...
    try {
      const yamlContent = exportAgentConfigToYAML(config.name, config.description, config.agents);
      const filename = generateAgentConfigExportFilename(config.name);
      downloadYAMLFile(yamlContent, filename);
      toast.success(`Configuration "${config.name}" exported successfully!`);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      toast.error(`Failed to export configuration: ${errorMessage}`);
      console.error(error);
    }
  };

  const handleImportAgentConfig = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;

    try {
      const content = await readFileAsText(file);
      const configData = importAgentConfigFromYAML(content);
      
      // Pass existing API configs so agents can be mapped to them
      const existingConfigs = getApiConfigs();
      const imported = importAgentConfiguration(configData, existingConfigs);
      
      setAgentConfigurations(getAgentConfigurations());
      
      // Notify parent to refresh agents
      onAgentConfigChange?.();
      
      // Check how many agents were successfully mapped to existing configs
      const mappedCount = imported.agents.filter(agent => 
        existingConfigs.some(config => config.id === agent.apiConfigId && config.isValidated)
      ).length;
      
      const unmappedCount = imported.agents.length - mappedCount;
      
      if (mappedCount === imported.agents.length) {
        toast.success(`Configuration "${imported.name}" imported with ${imported.agents.length} agents, all mapped to validated API configs!`);
      } else if (mappedCount > 0) {
        toast.warning(
          `Configuration "${imported.name}" imported with ${imported.agents.length} agents. ` +
          `${mappedCount} mapped to validated configs, ${unmappedCount} need API configuration.`,
          { duration: 5000 }
        );
      } else {
        toast.warning(
          `Configuration "${imported.name}" imported with ${imported.agents.length} agents. ` +
          `All agents need API configuration. Please add and validate API configs in settings.`,
          { duration: 5000 }
        );
      }
      
      // Suggest user to reload if active config was switched
      if (getActiveConfigId() !== imported.id) {
        setTimeout(() => {
          toast.info('To use the imported configuration, switch to it and reload the page.');
        }, 1000);
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      toast.error(`Failed to import agent configuration: ${errorMessage}`);
      console.error(error);
    } finally {
      if (agentConfigFileInputRef.current) {
        agentConfigFileInputRef.current.value = '';
      }
    }
  };

  const handleSwitchAgentConfig = (configId: string) => {
    setActiveConfigId(configId);
    setActiveConfigIdState(configId);
    toast.success('Agent configuration switched. Reload to see changes.');
  };

  const handleDuplicateAgentConfig = (configId: string) => {
    const duplicated = duplicateAgentConfiguration(configId);
    if (duplicated) {
      setAgentConfigurations(getAgentConfigurations());
  ...
  const handleDeleteAgentConfig = (configId: string) => {
    ...
    deleteAgentConfiguration(configId);
    setAgentConfigurations(getAgentConfigurations());
    ...
    >
      <div className="space-y-6">
        ...
        <div className="border rounded-lg p-4 bg-muted/30">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Import a previously exported session to restore all messages, agent states, and configuration.
            You can export sessions individually from the session list.
          </p>
          <div className="flex gap-2">
            <Button
              variant="outline"
          ...
          <p className="text-xs text-muted-foreground mt-2">
            💡 Tip: Export individual sessions using the download button next to each session in the sidebar.
          </p>
        </div>

        {/* Agent Configurations Section */}
        <div className="border rounded-lg p-4 bg-muted/30">
          <h3 className="text-lg font-semibold mb-3">Agent Configurations</h3>
          <p className="text-sm text-muted-foreground mb-4">
            Manage different agent team configurations. Create, import, export, and switch between different agent setups.
          </p>
          <div className="flex gap-2 mb-4">
            <Button
              variant="outline"
              onClick={() => agentConfigFileInputRef.current?.click()}
              className="flex items-center gap-2"
            >
              <Upload className="w-4 h-4" />
              Import Agent Config
            ...
            <input
              ref={agentConfigFileInputRef}
              type="file"
              accept=".yaml,.yml"
              onChange={handleImportAgentConfig}
              style={{ display: 'none' }}
            />
          </div>
          ...
          <div className="space-y-2">
            {agentConfigurations.map((config) => (
              <div
                key={config.id}
                className={`border rounded-lg p-3 ${config.id === activeConfigId ? 'ring-2 ring-primary bg-primary/5' : ''}`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {config.agents.length} agents • Updated {new Date(config.updatedAt).toLocaleDateString()}
                    </p>
                  </div>
                  <div className="flex items-center gap-1">
                    {config.id !== activeConfigId && (
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleSwitchAgentConfig(config.id)}
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => handleExportAgentConfig(config.id)}
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => handleDuplicateAgentConfig(config.id)}
                    ...
                    {config.id !== activeConfigId && (
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleDeleteAgentConfig(config.id)}
            ...
            {agentConfigurations.length === 0 && (
              <div className="text-center py-4 text-muted-foreground text-sm">
                <p>No agent configurations yet. Import one to get started.</p>
              </div>
            )}
          </div>
        </div>
