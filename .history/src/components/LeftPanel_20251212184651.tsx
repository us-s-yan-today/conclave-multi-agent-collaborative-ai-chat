        ...
        <div className="p-3 sm:p-4 space-y-4 sm:space-y-6 w-full max-w-full overflow-hidden">
          <div className="w-full max-w-full overflow-hidden">
            <div className="flex items-center justify-between gap-2 mb-2 w-full">
              <h2 className="text-sm font-semibold uppercase text-muted-foreground tracking-wider truncate flex-shrink">Agents</h2>
                ...
                <Select onValueChange={id => {
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <SelectTrigger className="h-7 w-[90px] text-xs px-2 border-dashed">
          ...
          <div className="w-full max-w-full overflow-hidden">
            <div className="flex items-center justify-between gap-2 mb-2 w-full">
              <h2 className="text-sm font-semibold uppercase text-muted-foreground tracking-wider truncate flex-shrink">Sessions</h2>
              <div className="flex-shrink-0">
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-7 px-2 text-xs border-dashed border flex-shrink-0 whitespace-nowrap"
            ...
            <div className="space-y-1 w-full max-w-full overflow-hidden">
              {sessions.map(session => (
                <div key={session.id} className={cn("group flex items-center justify-between p-2 rounded-lg cursor-pointer transition-colors hover:bg-accent dark:hover:bg-muted/50 w-full max-w-full overflow-hidden", currentSessionId === session.id && "bg-accent dark:bg-muted/50 border-l-2 border-primary")}>
                  <button onClick={() => onSwitchSession(session.id)} className="flex-1 text-left text-xs sm:text-sm truncate pr-2 font-medium overflow-hidden">
                    {session.title}
                  </button>
                  <Tooltip>
                    <TooltipTrigger asChild>
